...
from omniserve.modeling.layers.quantized_linear import W4A8OF16LinearDynamicInputScale
...
class LlamaMLP(nn.Module):
    def __init__(self, args, model_config, group_size: int) -> None:
        super().__init__()
...
class LlamaAttention(nn.Module):
    def __init__(
        ...
        # self.kv_scale_orig_quant = nn.Parameter(torch.ones(2))
...
class LlamaModel(nn.Module):
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                (
                    LlamaDecoderLayer(config, model_config, group_size, i, kv_cache_config)
                    if quant_kv_cache
                    else None
                )

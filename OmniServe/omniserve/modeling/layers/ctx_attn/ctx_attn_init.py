...
def init_ctx_sparse_attn(
    ...
    for idx, layer in enumerate(model.model.layers):
        ...
        layer_full_attention_heads = torch.as_tensor(full_attention_heads[idx], device=device, dtype=dtype)
        if not sparse_kv_cache_enabled or not sparse_context_enabled or torch.all(layer_full_attention_heads == 1):
            ...
            streaming_info = torch.as_tensor(
...
def init_sparse_kv_cache(
    ...
    for idx, layer in enumerate(model.model.layers):
        module = layer.self_attn
        layer_full_attention_heads = torch.as_tensor(full_attention_heads[idx], device=device, dtype=torch.int32)

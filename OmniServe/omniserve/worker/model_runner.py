...
class ModelRunner:
    ...
    def __init__(
        ...
        # )
        # GPU blocks 변수를 맨 먼저 초기화 (UnboundLocalError 완전 방지)
        num_retrieval_gpu_blocks = 3000  # 기본값 (Multi-GPU 환경에 맞춰 조정)
        num_streaming_gpu_blocks = 25   # 기본값 (Multi-GPU 환경에 맞춰 조정)
        ...
        # 최종 할당 전 변수 상태 확인
        print(f"[DEBUG] num_retrieval_gpu_blocks = {num_retrieval_gpu_blocks}")
        print(f"[DEBUG] num_streaming_gpu_blocks = {num_streaming_gpu_blocks}")

"""
Original Inefficient Trace Generator (based on gen_trace_mos.py)
- Same as gen_trace_mos.py but with 2 core inefficiencies:
  1. No row locality optimization (no row address sorting)
  2. Force traditional K-V execution (page_scheduling=False always)
- All other optimizations (deduplication, adaptive overlapping, etc.) preserved
- Perfect comparison baseline against MOS optimizations
"""
...
def Attention(L: int, key_addr: int, val_addr: int, itr: int, page_size: int,
    ...
    def score_mac(addr_offset: int, L: int):
        ...
        # INEFFICIENCY 1: No row locality optimization (removed row sorting)
        # TODO: Toggle this for testing - uncomment to enable row sorting like MOS
        valid_n_enhanced.sort(key=lambda x: x['row_addr'])  # TEMPORARILY ENABLED for testing
        ...
        for entry in valid_n_enhanced:
            ...
            # Cache expensive calculations outside inner loop
            region_base = get_region_base_address(addr_offset)
            ...
            for k_idx in range(k_range):
                for lch in range(math.ceil(valid_channel)):
                    ...
                    # Remove expensive logging for performance (keep inefficient access pattern)
                    # analyzer.log_page_mapping(...) # REMOVED for speed
    ...
    def context_mac(addr_offset: int, L: int):
        ...
        def process_k_indices(K_steps, per_k_col_factor, mac_opcode):
            for n_idx in range(math.ceil(dhead / per_k_col_factor)):
                ...
                # INEFFICIENCY 1: No row locality optimization (removed row sorting)
                # TODO: Toggle this for testing - uncomment to enable row sorting like MOS
                valid_k_enhanced.sort(key=lambda x: x['row_addr'])  # TEMPORARILY ENABLED for testing
                ...
                # Cache expensive calculation outside loop
                region_base = get_region_base_address(addr_offset)
...
def main():
    ...
    p = argparse.ArgumentParser(description="ORIGINAL INEFFICIENT AttAcc trace generator (based on MOS with 2 inefficiencies)",
    ...
    for k, v in vars(args).items():
        print(f"  {k}: {v}")
    print("---------------------------------------------------")
    print("🔴 ORIGINAL INEFFICIENT VERSION (based on MOS)")
    print("INEFFICIENCIES:")
    print("  1. No row locality optimization (no row sorting)")  
    print("  2. Traditional K-V scheduling only (no page-first)")
    print("---------------------------------------------------")
    ...
    # Run
    # INEFFICIENCY 2: Force traditional K-V scheduling (ignore page_scheduling argument)
    print("📊 ORIGINAL INEFFICIENT: Traditional scheduling (all-K → SFM → all-V) - page_scheduling FORCED to False")

...
class System:
    ...
    def set_accelerator(self, modelinfos, name: DeviceType, config, mod_params=None):
        self.hetero_name = name
        if self.hetero_name == DeviceType.PIM:
            # Use mod alignment parameters if provided
            if mod_params:
                ramulator = Ramulator(modelinfos, "ramulator2", "ramulator.out",
                                    mod_score=mod_params.get('mod_score', 1),
                                    mod_context=mod_params.get('mod_context', 1),
                                    mac_mode=mod_params.get('mac_mode', 'buffer'),
                                    page_scheduling=mod_params.get('page_scheduling', False),
                                    page_size=mod_params.get('page_size', 64),
                                    csv_file=mod_params.get('csv_file', ''),
                                    output_trace=mod_params.get('output_trace', ''),
                                    use_ori=mod_params.get('use_ori', False))
            else:
                ramulator = Ramulator(modelinfos, "ramulator2", "ramulator.out")
